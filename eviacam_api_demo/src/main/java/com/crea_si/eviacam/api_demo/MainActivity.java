...
public class MainActivity extends Activity implements
    ...
    public static final int SWIPE_VELO_THERESH = 100;
    ...
    public boolean onFling(MotionEvent downEvent, MotionEvent moveEvent, float velocityX, float velocityY){
        ...
        // which is greater? movemetn across Y or X?
        if (Math.abs(diffX) > Math.abs(diffY)){
            //right or reft swipe
            if (Math.abs(diffX) > SWIPE_THRESH && Math.abs(velocityX) > SWIPE_VELO_THERESH) {
                if (diffX > 0) {
                    onSwipeRight ();
                } else {
                    onSwipeLeft ();
            ...
            // up or down swipe
            if (Math.abs(diffY) > SWIPE_THRESH && Math.abs(velocityY) > SWIPE_VELO_THERESH){
                if (diffY > 0 ) {
                    onSwipeBottom ();
                }else{
                    onSwipeTop();
